...
optparse = OptionParser.new do |opts|
  ...
  opts.on("-g","--group MG","show only machines thet belongs to a mchinegroup. Wildcards allowd (*,?).") do |n|
    options[:machinegroup] = n
  end
...
machgrp = options[:machinegroup].to_s
if machgrp.empty?
  machgrp = '*'
end
logger.info "MachineGroup: #{machgrp}"

#logger.info "#{t}"
logger.info "#{serverurlport}"
Excon.defaults[:ssl_verify_peer] = false
response = Excon.get("https://#{serverurlport}/api/machines?sortBy=name&groupName=#{machgrp}", :headers => {'Authorization' => "Token #{t}" })
puts response.body
